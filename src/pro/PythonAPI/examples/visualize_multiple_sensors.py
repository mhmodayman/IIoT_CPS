...
class SensorManager:
    def __init__(self, world, display_man, sensor_type, transform, attached, sensor_options, display_pos):
        ...
        self.obstacle_sensor_event_count = 0
    ...
    def init_sensor(self, sensor_type, transform, attached, sensor_options):
        ...
        elif sensor_type == "DistanceToObstacles":
            obstacle_bp = self.world.get_blueprint_library().find('sensor.other.obstacle')
            for key in sensor_options:
                obstacle_bp.set_attribute(key, sensor_options[key])

            # obstacle_bp.set_attribute('hit_radius', '0.5')
            # obstacle_bp.set_attribute('distance', '50')

            obstacle_sensor = self.world.spawn_actor(obstacle_bp, transform, attach_to=attached)
            obstacle_sensor.listen(lambda event: self.print_distance_to_object(event))
    ...
    def print_distance_to_object(self, event):
        self.obstacle_sensor_event_count += 1
        print("Event %s, in line of sight with %s at distance %u" % (self.obstacle_sensor_event_count, event.other_actor.type_id, event.distance))
...
def get_front_bamper_midpoint(actor):
    center_point = actor.get_location()  # center point on earth usually (0, 0, 0)

    x_center = center_point.x
    y_center = center_point.y
    z_center = center_point.z

    bb = actor.bounding_box.extent
    front_bamper_midpoint = x_center + (bb.x / 2), y_center, z_center

    return front_bamper_midpoint


# def spawn_pedestrians(world):
#     ped_blueprints = world.get_blueprint_library().filter("walker.*")
#     player = world.try_spawn_actor(random.choice(ped_blueprints), carla.Transform())
...
def run_simulation(args, client):
    ...
    try:
        ...
        x_front, y_front, z_front = get_front_bamper_midpoint(vehicle)
        ...
        SensorManager(world, display_manager, 'DistanceToObstacles',
                      carla.Transform(carla.Location(x=x_front, z=0.3), carla.Rotation(yaw=+00)),
                      vehicle, {'hit_radius' : '0.2', 'distance' : '3'}, display_pos=[0, 1])
